...
export const getApiKeysHeaders = () => {
  const headers = {};
  try {
    const stored = localStorage.getItem("user_api_keys");
    if (stored) {
      const parsed = JSON.parse(stored);
      if (parsed.openai) headers["X-OpenAI-API-Key"] = atob(parsed.openai);
      if (parsed.claude) headers["X-Claude-API-Key"] = atob(parsed.claude);
      if (parsed.gemini) headers["X-Gemini-API-Key"] = atob(parsed.gemini);
    }
  } catch (err) {
    console.warn("Failed to retrieve API keys", err);
  }
  return headers;
};
...
axiosInstance.interceptors.request.use((config) => {
  ...
  // Attach user-supplied API keys from localStorage
  const keyHeaders = getApiKeysHeaders();
  Object.assign(config.headers, keyHeaders);
